    ...
    while(!$done && $limit > 0) {
      ...
      if ($entry->loaded()) {
        ...
        foreach ($child_paths as $child_path) {
            try {
//Folder_Sync_Controller::log("Syncing:  $child_path");
          $name = basename($child_path);
          $title = item::convert_filename_to_title($name);

          if (is_dir($child_path)) {
Folder_Sync_Controller::log("Syncing dir:  $child_path");
...
//Folder_Sync_Controller::log("Syncing $ext file:  $child_path");
            ...
            if($entry_exists && $entry_exists->loaded())
            {
//Folder_Sync_Controller::log("Syncing existing file:  $child_path");
...
Folder_Sync_Controller::log("Syncing new photo:  $child_path");
...
Folder_Sync_Controller::log("Syncing new movie:  $child_path");
                $item = ORM::factory("item");
                $item->type = "movie";
                $item->parent_id = $parent->id;
                $item->set_data_file($child_path);
...
Folder_Sync_Controller::log("Error adding file $child_path: " . $e->getMessage());
            }
          // Did we hit the limit?
          if($limit <= 0) {
Folder_Sync_Controller::log("Ending on limit");
            exit;
    	  }
        }
...
Folder_Sync_Controller::log("Ending...");
  ...
  public function log($msg) {
      $f = fopen(VARPATH . '/logs/folder_sync.log', 'a');
