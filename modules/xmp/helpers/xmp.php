...
class xmp_Core {
  ...
  static function extract($item, $force_debug) {
    ...
    if( $xmpData === false) {
//LSL      message::warning("[XMP Module] Warning, couldn't read any XMP data from \"{$item->title}\". Please erase all current metadata, or ensure there is some, and try again.");
//LSL      Kohana_Log::add("warning", "[XMP Module] Warning, couldn't read any XMP data from \"{$item->title}\". Please erase all current metadata, or ensure there is some, and try again." );
